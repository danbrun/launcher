...
class LegacyLauncherIconDrawable
private constructor(private val icon: Drawable, private val palette: Palette) : Drawable() {

  private val background: GradientDrawable =
    GradientDrawable().apply { setColor(palette.getMutedColor(Color.WHITE)) }
  ...
  companion object {
    ...
    suspend fun create(icon: Drawable): LegacyLauncherIconDrawable =
      withContext(Dispatchers.IO) {
        LegacyLauncherIconDrawable(icon, Palette.from(icon.toBitmap()).generate())
      }
  }
}
